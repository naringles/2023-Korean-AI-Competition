...
def load_audio(
    audio_path: str, del_silence: bool = False, extension: str = "pcm"
) -> np.ndarray:
    """
...
def _signal_to_frame_nonsilent(
    ...
    mse = __rms(y=y_mono, frame_length=frame_length, hop_length=hop_length) ** 2
...
def split(y, top_db=60, ref=np.max, frame_length=2048, hop_length=512):
    ...
    edges = _frames_to_samples(np.concatenate(edges), hop_length=hop_length)
