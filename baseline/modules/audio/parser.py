...
class SpectrogramParser(AudioParser):
    ...
    def __init__(
        ...
        if transform_method.lower() == "mel":
            self.transforms = MelSpectrogram(
                sample_rate, n_mels, frame_length, frame_shift, feature_extract_by
            )
            ...
            self.transforms = Spectrogram(
                sample_rate, frame_length, frame_shift, feature_extract_by
            )
    ...
    def parse_audio(self, audio_path: str, augment_method: int) -> Tensor:
        ...
        """
        signal = load_audio(
            audio_path, self.del_silence, extension=self.audio_extension
        )
