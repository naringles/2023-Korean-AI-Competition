...
def bind_model(model, optimizer=None):
    ...
    nova.bind(save=save, load=load, infer=infer)
...
def inference(path, model, **kwargs):
    ...
    for i in glob(os.path.join(path, "*")):
        results.append(
            {"filename": i.split("/")[-1], "text": single_infer(model, i)[0]}
        )
...
if __name__ == "__main__":
    ...
    vocab = KoreanSpeechVocabulary(
        os.path.join(os.getcwd(), "labels.csv"), output_unit="character"
    )
    ...
    if config.mode == "train":
        ...
        train_dataset, valid_dataset = split_dataset(
            config, os.path.join(os.getcwd(), "transcripts.txt"), vocab
        )
        ...
        optimizer = Optimizer(
            optimizer,
            lr_scheduler,
            int(len(train_dataset) * config.num_epochs),
        ...
        for epoch in range(num_epochs):
            ...
            print(
                "[INFO] Epoch %d (Training) Loss %0.4f CER %0.4f"
            ...
            print(
                "[INFO] Epoch %d (Validation) Loss %0.4f  CER %0.4f"
